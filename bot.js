...
const { getUserUUID, getUser } = require('./user');
const { getQuestion, answerQuestion } = require('./questions');
...
const quiz = new WizardScene(
    ...
    async (ctx) => {
        ...
        if (txt) {
            ...
            await answerQuestion(ctx.update.message.from.id, templateReg[`${txt} `]);
        }
        const question = await getQuestion(ctx.update.message.from.id);
    ...
    (ctx) => {
        ctx.reply(`заебись, вот твои результат ${ctx.session.result}`);
